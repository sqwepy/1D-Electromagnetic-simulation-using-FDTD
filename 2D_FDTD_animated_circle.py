...
# Center of the grid
cy, cz = i_max // 2, j_max // 2
Y, Z = np.ogrid[:i_max, :j_max]

# Radius of the circle (in grid cells)
r = np.sqrt((Y - cy)**2 + (Z - cz)**2)

# Dielectric circle: radius 80 cells
eps[(r > 100) & (r < 150)] = (np.sqrt(10)*(1+1j)) * eps0

plt.imshow(eps.T > eps0, origin='lower', cmap='magma')
plt.title('Circular dielectric region')
plt.show()
...
ani.save(
    "2D_fdtd_mur_1st_deg_em_wave_donut_complex.mp4",        # filename
